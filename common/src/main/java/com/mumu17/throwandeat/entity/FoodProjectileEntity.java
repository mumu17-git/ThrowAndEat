...
public class FoodProjectileEntity extends ThrowableItemProjectile {
        ...
        if (!this.level().isClientSide) {
            ...
            if (result.getEntity() instanceof Player targetPlayer) {
                ...
                if (foodProperties != null) {
                    targetPlayer.getFoodData().eat(foodProperties.getNutrition(), foodProperties.getSaturationModifier());
                    for (Pair<MobEffectInstance, Float> possibleEffectPair : foodProperties.getEffects()) {
                        if (this.random.nextFloat() < possibleEffectPair.getSecond()) {
                            targetPlayer.addEffect(possibleEffectPair.getFirst());
                        }
                    }
    ...
    private void healMob(LivingEntity targetMob, ItemStack itemStack) {
        FoodProperties foodProperties = itemStack.getItem().getFoodProperties();
        if (foodProperties != null) {
            float healAmount = foodProperties.getNutrition() * 2.0F;

            targetMob.heal(healAmount);

            for (Pair<MobEffectInstance, Float> possibleEffectPair : foodProperties.getEffects()) {
                if (this.random.nextFloat() < possibleEffectPair.getSecond()) {
                    targetMob.addEffect(possibleEffectPair.getFirst());
                }
            }
